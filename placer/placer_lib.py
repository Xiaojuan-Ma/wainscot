...
#     "only_important_ops", True, "Use only important ops for the placement.")
#
...
class DefaultPlacer(Placer):
    ...
    def __init__(self, tf_graph, devices, method, comm_cost_fn,
        ...
        # xiao: add collecting data logic for pesto exp.
        if FLAGS.is_pesto and FLAGS.pesto_collect_data:
            print(f'collecting op_graph for pesto')
            filename = FLAGS.pesto_dir + f'{FLAGS.model_name}_colocation_graph_for_pesto_forward{FLAGS.only_forward}_colocateback{FLAGS.colocate_grads_with_ops}.pkl'
            with open(filename, 'wb') as file:
                pickle.dump(self.op_graph, file)
                print(f'data has been writen to {filename}')
            FLAGS.pesto_collect_data = False   # set flag to False to avoid unexpected exit for other exp
            exit()  # for pesto data collection, exit after get this file.
    ...
    def run(self):
        ...
        self.run_adjuster()
        print('what is the memory ratio in the placer run', FLAGS.placer_memory_fraction)
        # exit(34)
        # simulate execution
        ...
        #print("reallocator inside placer")
        #reallocator = Reallocator(self)
        #experiment = Reallocator_Experiment(reallocator)
        #experiment.run()
